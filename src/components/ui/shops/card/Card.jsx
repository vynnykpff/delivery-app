...
import {toggleCardStatus, toggleCardStatusInShop} from "../../../../store/status-card/statusCard.slice.js";
...
const Card = memo(function Card({image, name, price, id, shops}) {
	...
	return (
		<CardWrapper>
			...
			<CardButton className={statusCards[id] && statusCards[id].status ? 'addProductToCart' : 'removeProductFromCart'}
...
// 			dispatch(toggleCardStatus({ cardId: id, status: true }));
// 			dispatch(toggleCardStatusInShop({ cardId: id, status: true }));
// 			window.localStorage.setItem('Products', JSON.stringify(arrayProducts));
// 		}
// 	};
//
// 	const handleBuysCount = () => {
